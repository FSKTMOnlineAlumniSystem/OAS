...
import result from "./searchAlgo.js";
const alumniList = document.getElementById("alumniList");
let pageIndex = 0;

if (JSON.parse(localStorage.getItem("searchQuery")) == null) {
  ...
  //output +icon we cant find anything html empty
  console.log("cannot find result");
} else if (JSON.parse(localStorage.getItem("searchQuery")) != null) {
  const loadAlumniList = (pageIndex) => {
    alumniList.innerHTML = "";
    let alumniStartIndex = pageIndex * 10;
    let alumniEndIndex = alumniStartIndex + 10;
    ...
    var remainingLength = dataLength - alumniStartIndex;
    /*   js for button*/
    if (alumniEndIndex >= result.length) {
      document.getElementById("nextPage").innerHTML = `
        <li class="page-item disabled">
        <button id="nextPage"  onclick="nextPage()" class="page-link" tabindex="-1" aria-disabled="true">Next</button>
      </li>`;
    ...
    for (
      let i = alumniStartIndex;
      i < alumniEndIndex && i < result.length;
      i++
    ) {
      alumniList.innerHTML += `<div onclick="clickProfile(${i})" class="media justify-content-center mb-2 w-75 p-3" style="background-color:#E9E5E5;">
    <div class="image m-auto col-2 p-3">
    <div style="aspect-ratio:1/1;overflow:hidden;">
    <img src=${result[i].imageId} class="w-100" alt=${result[i].name}>
    </div>
    </div>
    <div class="media-body mr-3 my-auto col-10">
    <h6 class="mt-0 mb-1">${result[i].name}</h6>
    <em class="mb-0">Bachelor of Computer Science (${result[i].department}), graduated ${result[i].graduated}</em>
    <small style="display: -webkit-box;
    -webkit-line-clamp: 3;
    -webkit-box-orient: vertical;
    overflow: hidden;
    text-overflow: ellipsis;">${result[i].biography}</small>
    </div></div>
    </div>`;
    }
  };
